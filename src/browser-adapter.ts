...
const AUTH='https://accounts.google.com/v3/signin/identifier?continue=https%3A%2F%2Fnotebooklm.google.com%2F&flowName=GlifWebSignIn&flowEntry=ServiceLogin';
...
export class NotebookLmBrowserAdapter implements NotebookAdapter {
  ...
  private context?:BrowserContext; private page?:Page; private lockHandle?:Awaited<ReturnType<typeof open>>;
  ...
  async health(){
    try { const p=await this.getPage(); await p.goto(HOME,{waitUntil:'domcontentloaded',timeout:this.timeout}); if(!(await this.isAuthenticated(p)))return {ok:false,state:'AUTH_REQUIRED',detail:'NotebookLM session is not authenticated; run setup-auth.'}; return {ok:true,state:'READY'}; }
  ...
  async authenticate(interactive=false){
    try { await this.ensure(interactive,true); const p=await this.getPage(); await p.goto(AUTH,{waitUntil:'domcontentloaded',timeout:this.timeout});
      ...
      while(Date.now()<deadline){if(await this.isAuthenticated(p)){log('info','auth.verified');await this.close();return {ok:true,state:'READY',detail:`Persistent profile: ${this.profileDir}`};}await new Promise(r=>setTimeout(r,500));}
  ...
  private async openHome(){const p=await this.getPage();await p.goto(HOME,{waitUntil:'domcontentloaded',timeout:this.timeout});if(!(await this.isAuthenticated(p)))throw new Error('AUTH_REQUIRED');await p.waitForLoadState('networkidle',{timeout:Math.min(15000,this.timeout)}).catch(()=>undefined);return p;}
  ...
  private async ensure(interactive=false,force=false){if(this.context&&!this.context.pages().some(p=>!p.isClosed())){this.page=await this.context.newPage();return;}if(this.context)return;await mkdir(this.profileDir,{recursive:true});if(existsSync(this.lockFile)&&!force)throw new Error('PROFILE_LOCKED: another MCP/browser process owns the persistent profile');this.lockHandle=await open(this.lockFile,'wx').catch(()=>undefined);if(!this.lockHandle)throw new Error('PROFILE_LOCKED: persistent profile lock could not be acquired');const executable=process.env.NLM_CHROME_PATH??findChrome();this.context=await chromium.launchPersistentContext(this.profileDir,{headless:!(interactive||this.mode==='visible'),executablePath:executable||undefined,viewport:{width:1440,height:1000},args:['--disable-backgrounding-occluded-windows','--disable-renderer-backgrounding']});this.page=this.context.pages()[0]??await this.context.newPage();}
  ...
  private async close(){await this.context?.close().catch(()=>undefined);this.context=undefined;this.page=undefined;await this.lockHandle?.close().catch(()=>undefined);this.lockHandle=undefined;await rm(this.lockFile,{force:true}).catch(()=>undefined);}
}
