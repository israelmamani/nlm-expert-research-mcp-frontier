...
export class UpstreamNotebookLmAdapter implements NotebookAdapter {
  ...
  async shutdown(){const pid=this.transport?.pid;await this.transport?.close().catch(()=>undefined);this.transport=undefined;this.client=undefined;if(this.hideWatcher&&!this.hideWatcher.killed)this.hideWatcher.kill();this.hideWatcher=undefined;if(pid&&process.platform==='win32')await terminateProcessTree(pid);}
...
function operationBudget(name:string){if(name==='server_health'||name==='notebook_list')return 60_000;if(name==='content_list')return 90_000;if(name==='notebook_ask')return 180_000;if(name==='source_add'||name==='notebook_create'||name==='notebook_delete')return 240_000;return 120_000;}
function isTransportTimeout(error:unknown){return /timeout|timed out|deadline/i.test(String(error));}
async function terminateProcessTree(pid:number){await new Promise<void>(resolve=>{const child=spawn('taskkill.exe',['/PID',String(pid),'/T','/F'],{windowsHide:true,stdio:'ignore'});const timer=setTimeout(resolve,5_000);child.once('exit',()=>{clearTimeout(timer);resolve();});child.once('error',()=>{clearTimeout(timer);resolve();});});}
