...
export async function challenge(query:string, policy:InternetPolicy):Promise<ExternalResearch>{
  ...
  const safe=sanitizeExternalQuery(query); if(!safe)return {used:false,evidence:[],reason:'EXTERNAL_QUERY_REQUIRES_EXPLICIT_APPROVAL'};
  try {const url=`https://html.duckduckgo.com/html/?q=${encodeURIComponent(safe)}`;const res=await fetch(url,{headers:{'user-agent':'NLM-Expert-Research-MCP/0.1'},signal:AbortSignal.timeout(10000)});if(!res.ok)return {used:false,evidence:[],reason:`External HTTP ${res.status}`};const html=await res.text();const evidence:Evidence[]=[];for(const m of html.matchAll(/<a[^>]+class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/gi)){const title=clean(m[2]);let href='';try{href=decodeURIComponent(m[1]);}catch{continue;}if(!title||!href)continue;evidence.push({evidenceId:`ext_${evidence.length+1}`,sourceTitle:title,quote:href,layer:'CANDIDATE_EXTERNAL',status:'UNVERIFIED'});if(evidence.length>=5)break;}return {used:evidence.length>0,evidence,reason:'External results are discovery candidates and require explicit verification.'};}
...
export function sanitizeExternalQuery(q:string):string|null{const value=q.trim().replace(/\s+/g,' ');if(!value||value.length>160)return null;if(/["'`]/.test(value)||/\b(my|mi|mis|notebook|cuaderno|source|fuente|confidential|confidencial|internal|interno|private|privado|according to|seg[uú]n)\b/i.test(value))return null;const safe=value.replace(/[^\p{L}\p{N}\s:.,!?-]/gu,'').trim();return safe.length>=8?safe:null;}
