...
export function createApp(){
  ...
  server.registerTool('setup_auth',{description:'Open a visible Chrome window for secure Google login. Set force=true when cookies look valid locally but Google redirects to sign-in.',inputSchema:{force:z.boolean().optional()}},async a=>{const prior=process.env.NLM_FORCE_REAUTH;if(a.force)process.env.NLM_FORCE_REAUTH='1';try{return {content:[{type:'text',text:JSON.stringify(await adapter.authenticate(true),null,2)}]};}finally{if(prior===undefined)delete process.env.NLM_FORCE_REAUTH;else process.env.NLM_FORCE_REAUTH=prior;}});
